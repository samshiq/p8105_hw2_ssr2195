...
```{r table}
# Step 1: Create a summary table with the counts of Star Baker, Winner status, and Series
baker_summary_df = final_bakeoff_data_df |>
  filter(series >= 5 & series <= 10) |>  # Keep only series 5 to 10
  group_by(series, baker) |>              # Group by series and baker
  summarise(
    star_baker_count = sum(result == "STAR BAKER"),   # Count of times they were Star Baker
    winner = if_else(any(result == "WINNER"), "Yes", "No"),  # Check if they were a winner
    .groups = "drop"  # Drop grouping structure
  ) |>
  filter(star_baker_count > 0 | winner == "Yes") |>  # Keep entries where Star Baker count > 0 or baker is a winner
  arrange(series, desc(star_baker_count))  # Arrange in chronological order by series, then by descending Star Baker count
```

### Star Baker Counts and Winners (Seasons 5-10) with Counts > 0 unless Winner ###
```{r table_display}
# Step 2: Display the summary table
library(knitr)
baker_summary_df |> 
  kable(col.names = c("Series", "Baker", "Star Baker Count", "Winner")
  )
```
...
# Step 1: Load the viewership data and clean column names
viewers_long_df = read.csv("viewers.csv") |>
  janitor::clean_names() |>  # Clean column names

  # Step 2: Reshape data from wide to long format
  pivot_longer(
    cols = starts_with("series"),  # All columns starting with "series"
    names_to = "series",           # Create a new column called 'series'
    names_prefix = "series_",      # Remove the "series_" prefix
    values_to = "viewers"          # The values in the cells go to 'viewers'
  ) |>
  mutate(
    series = as.numeric(series)    # Convert 'series' to a numeric value
  ) |>
  drop_na(viewers) |>             # Remove rows with NA values in 'viewers' column
  arrange(series)                 # Arrange data by series in ascending order

# Step 3: Calculate the average viewership for Season 1 and Season 5
